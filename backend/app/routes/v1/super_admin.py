...
def create_user(data: CreateUserRequest, request: Request):
    ...
    try:
        ...
        elif data.role in ['teacher', 'hod', 'head_teacher', 'nurse', 'counselor', 'chaplain', 'social_worker', 'librarian']:
            sid = data.staff_id or IDGenerator.generate_entity_id(IDGenerator.PREFIXES.get(data.role, 'STF'), 0)
...
def list_users(role: str = None, search: str = None, page: int = Query(1, ge=1), limit: int = Query(20, ge=1, le=100)):
    ...
    params = []
    if role:
        query += " AND u.role = ?"; count_query += " AND role = ?"; params.append(role)
    if search:
...
def audit_log(user_id: int = None, action: str = None, page: int = Query(1, ge=1), limit: int = Query(20, ge=1, le=100)):
    ...
    params = []
    if user_id:
        query += " AND a.user_id = ?"; count_query += " AND user_id = ?"; params.append(user_id)
    if action:
        query += " AND a.action = ?"; count_query += " AND action = ?"; params.append(action)
